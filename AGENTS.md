...
| `NurseConfig` | `nurse/config.rs` | Master switches (`enabled`, `mode: Enabled\|Observe\|Disabled`, `nurse_model`, `nurse_provider`) + per-profile `ProfileConfig` map. `clamp_stall_threshold` lives here. `nurse_model = "none"` skips Tier 3 silently. The engine additionally carries `master_enabled` / `master_swarms_only` `Arc<AtomicBool>` mirrors of `config.enabled` / `config.swarms_only` so the sync synthesized path (`report_synthesized`) can gate without awaiting on the config RwLock; `commands::nurse::set_nurse_config` is the sole mutator and updates the config write + the mirrors in lock-step. `report_synthesized` consults both mirrors before dispatch (matching the dispatcher's `gated_disabled` / `gated_swarms_only` behaviour at `dispatcher.rs:520` / `:610`); when gated, it writes a symmetric decision-log pair and returns `None` instead of emitting a `nurse-event` Lifecycle. |
